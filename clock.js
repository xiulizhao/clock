(function (root, factory) {
   ...
   */
  function colck_btn($ele) {
    var config = {};
    config.hwidth = eval($ele.attr("data-hwidth")) || "8";     //设置时钟时针的宽度
    config.hheight = eval($ele.attr("data-hheight")) || "100"; //设置时钟时针的长度
    config.mwidth = eval($ele.attr("data-mwidth")) || "6";     //设置时钟分针的宽度
    config.mheight = eval($ele.attr("data-mheight")) || "180"; //设置时钟分针的长度
    config.swidth = eval($ele.attr("data-swidth")) || "4";     //设置时钟秒针的宽度
    config.sheight = eval($ele.attr("data-sheight")) || "260"; //设置时钟秒针的长度
    config.hcolor = $ele.attr("data-hcolor") || "#000";     //设置时钟时针的颜色
    config.mcolor = $ele.attr("data-mcolor") || "#000";     //设置时钟分针的颜色
    config.scolor = $ele.attr("data-scolor") || "#f00";     //设置时针秒针的颜色
    config.bpcolor = $ele.attr("data-bpcolor") || "#000";   //设置时钟表盘的颜色
    config.centercolor = $ele.attr("data-centercolor") || "#42deff"; //设置时钟中心圆点的颜色
    config.clockwidth = eval($ele.attr("data-clockwidth")) || "600"; //设置时钟宽度
    config.clockheight = eval($ele.attr("data-clockheight")) || "600"; //设置时钟的高度
    config.hkwidth = eval($ele.attr("data-hkwidth")) || "8";           //设置时钟小时刻度的宽度
    config.hkheight = eval($ele.attr("data-hkheight")) || "25";        //设置时钟小时刻度的高度
    config.mkwidth = eval($ele.attr("data-mkwidth")) || "6";           //设置时钟分钟刻度的宽度
    config.mkheight = eval($ele.attr("data-mkheight")) || "15";        //设置时钟分钟刻度的高度
    var vheight = (config.clockwidth / 2).toString();

    $(function () {
      var clock = document.getElementsByClassName("clock")[0];
      draw();
      var time = new Date();
      var h = drawpointer(config.hwidth, config.hheight, config.hcolor, time.getHours() * 30 + time.getMinutes() * 6 / 12);
      var m = drawpointer(config.mwidth, config.mheight, config.mcolor, time.getMinutes() * 6);
      var s = drawpointer(config.swidth, config.sheight, config.scolor, time.getSeconds() * 6);
      ...
      function draw() {
        for (var i = 0; i < 60; i++) {
          ...
          newdiv.style.cssText = "width:" + m + "px;height:" + h + "px;background:" + config.bpcolor + ";position:absolute;left:0;top:0;transform:translate(" + (config.clockwidth - m) / 2 + "px,0) rotate(" + i * 6 + "deg);transform-origin:" + m / 2 + "px " + vheight + "px"
      ...
      function drawpointer(w, h, b, d) {
        ...
        newd.style.cssText = "width:20px;height:20px;border-radius:50%;background: radial-gradient(circle at 50% 50%," + config.centercolor + ",royalblue);position:absolute;left:" + (config.clockwidth - 20) / 2 + "px;top:" + (config.clockwidth - 20) / 2 + "px;"
        newdiv.style.cssText = "width:" + w + "px;height:" + h + "px;position:absolute;left:" + (config.clockwidth - w) / 2 + "px;top:" + (vheight - h) + "px;background:" + b + ";transform:rotate(" + d + "deg);transform-origin:center bottom;";
        clock.appendChild(newdiv);
